...
void main() {
  ...
  group('MqttService', () {
    ...
    setUp(() {
      Logger.info("Setting up MQTT service test instance", tag: 'MQTTServiceTest');
    ...
    test('initializes with correct configuration', () {
      ...
      Logger.debug("MQTT config: ${mqttService.host}:${mqttService.port}", tag: 'MQTTServiceTest');
      Logger.info("MQTT service configuration test passed", tag: 'MQTTServiceTest');
    });

    test('generates dummy sensor data correctly', () {
      Logger.info("Testing MQTT service dummy data capabilities", tag: 'MQTTServiceTest');
      ...
      Logger.debug("Service created with host: ${service.host}", tag: 'MQTTServiceTest');
      Logger.info("Dummy data generation capabilities test passed", tag: 'MQTTServiceTest');
    });

    test('streams are available and properly typed', () {
      Logger.info("Testing MQTT service stream availability", tag: 'MQTTServiceTest');
      ...
      Logger.info("All MQTT streams are properly typed and available", tag: 'MQTTServiceTest');
    });

    group('device command publishing', () {
      test('formats device command correctly', () async {
        Logger.info("Testing device command formatting", tag: 'MQTTServiceTest');
        ...
        Logger.debug("Command result type: ${result.runtimeType}", tag: 'MQTTServiceTest');
        
        expect(result, isA<Failure>());
        expect((result as Failure).error, isA<MqttError>());
        
        Logger.info("Device command formatting test passed (expected failure due to no connection)", tag: 'MQTTServiceTest');
      });

      test('handles command without parameters', () async {
        Logger.info("Testing device command without parameters", tag: 'MQTTServiceTest');
        ...
        Logger.info("Command without parameters test passed (expected failure)", tag: 'MQTTServiceTest');
      });
    });

    group('dummy data generation', () {
      test('sensor types have correct value ranges', () {
        Logger.info("Testing sensor value range generation", tag: 'MQTTServiceTest');
        ...
        Logger.debug("Service client ID: ${service.clientId}", tag: 'MQTTServiceTest');
  ...
  group('MqttService Integration', () {
    ...
    test('mock service can simulate device command publishing', () async {
      Logger.info("Testing mock device command publishing", tag: 'MQTTMockTest');
      ...
      Logger.info("Mock device command publishing test passed", tag: 'MQTTMockTest');
    });
    ...
    test('mock service provides sensor data stream', () {
      ...
      Logger.debug("Mock sensor data: ${sensorData.sensorType.name} = ${sensorData.value}", tag: 'MQTTMockTest');
    ...
    test('mock service provides device status stream', () {
      ...
      Logger.debug("Mock device: ${device.name} (${device.status.name})", tag: 'MQTTMockTest');
