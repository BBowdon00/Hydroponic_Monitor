...
Future<HttpServer> _startServer(List<List<int>> frames, {String boundary = 'testboundary'}) async {
  final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
  server.listen((HttpRequest request) async {
    request.response.statusCode = 200;
    request.response.headers.set('Content-Type', 'multipart/x-mixed-replace; boundary=$boundary');
...
void main() {
  group('MjpegStreamController Parser', () {
    test('parses two frames successfully', () async {
      final frames = [
        _jpegStub(1).toList(),
        _jpegStub(2).toList(),
      ];
