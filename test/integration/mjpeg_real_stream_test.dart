...
void main() {
  group('Real MJPEG Stream Integration (feature-flagged)', () {
    test('streams frames and updates state', () async {
      if (!Env.enableRealMjpeg) {
        print('Skipping real stream test: feature flag disabled');
        return;
      }
      final server = await FakeMjpegServer.start([
        FakeMjpegFrameSpec(buildStubJpeg(1), delay: const Duration(milliseconds: 50)),
        FakeMjpegFrameSpec(buildStubJpeg(2), delay: const Duration(milliseconds: 50)),
        FakeMjpegFrameSpec(buildStubJpeg(3), delay: const Duration(milliseconds: 50)),
      ]);
    ...
    test('handles invalid server boundary', () async {
      ...
      server.listen((req) async {
        ...
        req.response.headers.set('Content-Type', 'multipart/x-mixed-replace'); // missing boundary
