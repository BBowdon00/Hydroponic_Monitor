...
void main() {
  group('MQTT → Telegraf → InfluxDB Integration', () {
    ...
    setUpAll(() async {
      ...
      mqttClient = MqttServerClient(TestConfig.testMqttHost, 'integration_test_publisher');
    ...
    test('End-to-end data flow: MQTT publish → Telegraf → InfluxDB storage', tags: ['integration'], () async {
      ...
      expect(mqttClient.connectionStatus!.state, equals(MqttConnectionState.connected));
    ...
    test('Multiple sensor types data flow', tags: ['integration'], () async {
      ...
      expect(storedCount, greaterThanOrEqualTo(testDataList.length ~/ 2), 
...
Future<void> _waitForInfluxDB() async {
  for (int i = 0; i < 30; i++) {
    try {
      final response = await http.get(Uri.parse('${TestConfig.testInfluxUrl}/health'));
...
Future<void> _waitForMQTT() async {
  for (int i = 0; i < 30; i++) {
    try {
      final socket = await Socket.connect(TestConfig.testMqttHost, TestConfig.testMqttPort);
      socket.destroy();
...
''';
    ...
    final response = await http.post(
      Uri.parse('${TestConfig.testInfluxUrl}/api/v2/query?org=${TestConfig.testInfluxOrg}'),
      headers: {
